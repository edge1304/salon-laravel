...
trait HelperTrait{
    ...
    public function calculate_money($price = 0, $vat = 0, $ck = 0, $discount = 0, $quantiy = 1){
        $price = $this->tryParseInt($price);
        $vat = $this->tryParseInt($vat);
        $ck = $this->tryParseInt($ck);
        $discount = $this->tryParseInt($discount);
        $quantiy = $this->tryParseInt($quantiy);

        return ($price + $price/100*$vat - $price/100*$ck - $discount)*$quantiy;
    }
